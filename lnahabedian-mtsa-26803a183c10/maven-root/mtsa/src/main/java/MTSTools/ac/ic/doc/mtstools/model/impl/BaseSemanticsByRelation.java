...
public class BaseSemanticsByRelation implements ImplementationNotion, RefinementByRelation {
	...
	public <S1, S2, A> boolean isARefinement(MTS<S1, A> mts1, MTS<S2, A> mts2) {
		return this.isARefinement(mts1, mts2, mts1.getInitialState(), mts2.getInitialState());
	}
	...
	public <S1, S2, A> boolean isARefinement(MTS<S1, A> mts1, MTS<S2, A> mts2, S1 state1, S2 state2) {
		Validate.isTrue(areComparable(mts1, mts2), "The models are not comparable");
		return this.getRelationConstructor()
			.getLargestRelation(mts1, mts2).contains(Pair.create(state1, state2));
	}
