...
public class TransitionSystemDispatcher {
	...
	public static void synthesiseSyncController(CompositeState compositeState, LTSOutput output) {
		...
		if (composition != null) {
			if (compositeState.goal != null) {
				CompactState synthesiseController = synthesiseSyncController(compositeState, compositeState.goal,
						output);
	...
	public static CompactState synthesiseSyncController(CompositeState compositeState, GRControllerGoal<String> goal,
			LTSOutput output) {
		...
		// TODO preprocess must be pulled into to SynchronousControlProblem
		// construction.
		// That requires ControllersUtils to be generic and placed in
		// MTSSynthesis project
		...
		MTS<Long, String> synchronous = ControllerUtils.getSynchronous(env, goal.getControllableActions(), 0L, 1L,
				envYieldsAction, contYieldsAction);
		env = ControllerUtils.embedFluents(synchronous, goal, output);

		// TODO refactor needed. there is a problem with the generics we're
		// using. It shouldn't be so hard to work with LTS/MTS
		// interchangeably and also there is an issue with the generic to MTS to
		// be fixed.

		// Synthesis Process
		SynchronousControlProblem<Long, String> cp = new SynchronousControlProblem<Long, String>(env, goal,
				envYieldsAction, contYieldsAction);
		LTS<StrategyState<Long, Integer>, String> synthesised = new ControllerSynthesisFacade<Long, String, Integer>()
		...
		if (synthesised == null) {
			...
			MTSAdapter<StrategyState<Long, Integer>, String> mtsAdapter = new MTSAdapter<StrategyState<Long, Integer>, String>(
					synthesised);
	...
	// ToDani: este metodo deberia ser synthesise y resolver que control problem
	// hay que aplicar.
	public static CompactState synthesiseGR(CompositeState compositeState, GRControllerGoal<String> goal,
			LTSOutput output) {
	...
	public static CompactState synthesiseGRNoText(CompositeState compositeState, GRControllerGoal<String> goal,
			LTSOutput output) {
	...
	private static void recordControllerStateLabels(Map<StrategyState<Long, Integer>, Long> mapping,
			GRGame<Long> lastGame, GRControllerGoal<String> goal) {
	...
	public static void makeProperty(CompositeState compositeState, LTSOutput output) {
        if (compositeState.getComposition() != null) {
            if (!MTSUtils.isMTSRepresentation(compositeState)) {
                ...
                String[] alphabetArray = currentComposition.getPureAlphabet();
...
//				}
                    ...
                    if (state != -1) {
						...
						for (String transitionLabel : posibleTransitions) {
							if (compositeState.actionsToErrorSet == null || compositeState.actionsToErrorSet.contains(transitionLabel)) {
								mts.addTransition(state, transitionLabel, new Long(-1), TransitionType.REQUIRED);
							}
						}
					}
				}
	...
	public static CompactState synthesisePriorityGR(CompositeState compositeState, GRControllerGoal<String> goal,
											LTSOutput output) {
		...
		try {
			...
			for (Formula formula : formulas) {
				...
				newGoal.addGuarantee(formula);
				newGoal.getFluents().clear();
				Fluent actualFluent = null;

				for(Fluent fluent : fluents) {
					if (formula.toString().equals(fluent.getName()))
						actualFluent = fluent;
				}
				...
				compositeState.goal = newGoal;

				CompactState actualComposite = synthesiseGR(compositeState, newGoal, output);

				if (actualComposite != null)
					result = actualComposite;
			}
		}catch (Exception e){
			e.printStackTrace();
		}
