...
public abstract class GRGameSolver<S> extends RankBasedGameSolver<S, Integer> {
	...
	public int getNextGuarantee(int guaranteeId, S state) {
		if (getGRGoal().getGuarantee(guaranteeId).contains(state) || getGRGoal().getFailures().contains(state)) {
	...
	public Strategy<S, Integer> buildStrategy() {
		...
		for (S state : winningStates) {
			for (int guaranteeId = 1; guaranteeId <= getGRGoal().getGuaranteesQuantity(); guaranteeId++) {
				...
				boolean rankMayIncrease = getGRGoal().getGuarantee(guaranteeId).contains(state)
